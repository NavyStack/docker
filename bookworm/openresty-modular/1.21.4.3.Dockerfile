ARG OPENRESTY_DOCKER_VERSION=1.25.3.2-jammy

FROM openresty/openresty:${OPENRESTY_DOCKER_VERSION} AS builder
...
WORKDIR /opt/build-stage/openresty-${OPENRESTY_VERSION}
...
RUN { \
        sed -i '1iload_module modules/ngx_pagespeed.so;' /usr/local/openresty/nginx/conf/nginx.conf; \
        sed -i '1iload_module modules/ngx_http_immutable_module.so;' /usr/local/openresty/nginx/conf/nginx.conf; \
        sed -i '1iload_module modules/ngx_http_cache_purge_module.so;' /usr/local/openresty/nginx/conf/nginx.conf; \
        sed -i '1iload_module modules/ngx_http_brotli_static_module.so;' /usr/local/openresty/nginx/conf/nginx.conf; \
        sed -i '1iload_module modules/ngx_http_brotli_filter_module.so;' /usr/local/openresty/nginx/conf/nginx.conf; \
    } \
    && sed -i 's+# server_tokens off+server_tokens off+g' /usr/local/openresty/nginx/conf/nginx.conf